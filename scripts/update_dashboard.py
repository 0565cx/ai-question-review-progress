...
# 语言问题审核的 2 个子文件夹（顺序即流程）
LANG_SUB = ["审核结果", "重出结果"]
LANG_DIRS = {
    "审核结果": "8_语言问题审核/1_审核结果",
    "重出结果": "8_语言问题审核/2_重出结果",
}
# 看板每轮展示的 5 个勾列
...
def round_of(name: str):
    """从文件名解析轮次号。支持阿拉伯数字多位数（如待10轮/第10轮）。"""
    m = re.search(r"待(\d+)轮审核", name)
    if m:
        return int(m.group(1))
    m = re.search(r"待([一二三四五六七八九十]+)轮审核", name)
    if m:
        return CN.get(m.group(1), 1)
    m = re.match(r"^(\d+)轮[_-]", name)
    if m:
        return int(m.group(1))
    m = re.search(r"第(\d+)轮", name)
    if m:
        return int(m.group(1))
    m = re.search(r"第([一二三四五六七八九十]+)轮", name)
    if m:
        return CN.get(m.group(1), 1)
    m = re.search(r"(\d+)轮", name)
    if m:
        return int(m.group(1))
    return None
...
def main():
    ...
    L.append("")
    L.append("> **知识审核后汇总**：知识审核全部轮次完成后保留的题目（● = 已传）。**语言审核**：2 个环节顺序为 ① 审核结果　② 重出结果。**最终汇总**：语言审核也完成后最终保留的题目（● = 已传）。")
    L.append("")
    L.append("## 总览")
    L.append("")
    ...
    L.append("| `8_语言问题审核/1_审核结果` | 语言问题审核结果 |")
    L.append("| `8_语言问题审核/2_重出结果` | 语言问题重出结果 |")
